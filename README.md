...
This repository ships an OpenCode multi-agent environment for Shopify development:

- **Agents** in `.opencode/agents/` — `shopify-architect` (orchestrator) plus
  specialists: `shopify-admin-api`, `shopify-app-backend`, `shopify-app-frontend`,
  `shopify-extensions`, `shopify-functions`, `shopify-webhooks`, `shopify-security`,
  `shopify-data-modeler`, and the `shopify-reviewer` final gate. Post-sale flow work
  uses the `postventa-*` agents documented in `AGENTS.md`.
- **Skills** in `.opencode/skills/` — vendored official Shopify AI Toolkit skills
  (search + validation scripts) plus a custom `shopify-webhooks` skill.
- **Shared rules** in `.opencode/instructions/` — source of truth, security, and
  validation policies loaded via `opencode.json`.
- **Update mechanism** — `scripts/update-shopify-ai-toolkit.mjs` keeps vendored skills
  in sync with upstream.

See [docs/agents/shopify-agent-architecture.md](./docs/agents/shopify-agent-architecture.md)
for the full architecture, delegation matrix, skills matrix, and update procedure.
